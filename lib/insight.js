 ...
 */
Insight.prototype.getUtxos = function(addresses, minconf, callback) {
  $.checkArgument(_.isFunction(callback));
 ...
 */
Insight.prototype.address = function(address, from, to, callback) {
  $.checkArgument(_.isFunction(callback));
  address = new Address(address);

  var params

  if (from && !NaN(from) && from > 0){
    params += '&from=' + from
  }
  ...
  this.requestGet('/api/addr/' + address.toString() + params, function(err, res, body) {
    if (err || res.statusCode !== 200) {
      return callback(err || body);
    }
    var info;
